...
const DropDownMenu = ({setOpen,authData}) => {

    const {setIndicator} = IndicatorData();

    return (
        <div style={{'--tw-bg-opacity': 1,backgroundColor: 'rgba(28,27,27,var(--tw-bg-opacity))',borderRadius:'15px'}}>
            <div style={{ position: 'relative' }}>
                <div style={{ position: 'absolute', top: '-10px', right: '22px', width: 0, height: 0, borderLeft: '5px solid transparent', borderRight: '5px solid transparent', borderBottom: '10px solid rgba(28, 27, 27, var(--tw-bg-opacity))' }}></div>
            </div>
            <ul style={{display:'block','--tw-text-opacity': 1,color:'rgba(201,201,201,var(--tw-text-opacity))',padding:'1rem',paddingTop:'1.25rem',listStyleType:'none'}}>
               {authData?.email&&<li style={{display:'flex',gap:'10px',marginBottom:'1rem',cursor:'text'}} className='headerpointer'>
                    <div style={{strokeWidth:2}}>
                ...
                {authData?.email&&<li style={{borderTop:'1px solid hsla(0,0%,100%,.2)',marginTop:'5px',marginBottom:'1rem'}}></li>}
                <li onClick={()=>{setOpen(false);setIndicator((prev)=>{return{...prev,linkIndicator:true}})}} style={{display:'flex',gap:'10px',marginBottom:'1rem'}} className='headerpointer'>
                    <div style={{strokeWidth:2}}>
